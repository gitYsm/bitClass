...
int main(){
    ...
    signal(SIGUSR2, chkAvail);
    signal(SIGINT, chkLastfolder);
    if ((ret = pthread_create(&p_thread[0], NULL, chkParent, NULL)) < 0)
	{
...
void chkAvail(int sig){
    ...
    while(dfget(MP))
    {
        if(strcmp(MP->mountdir, "/") == 0){
            ...
            fflush;
            if((double)MP->size.avail*100/MP->size.blocks < 10){
                printf("용량 30%% 미만 : 삭제\n");
                doRmDir(sig);
            }
        }
